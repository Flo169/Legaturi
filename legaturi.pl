/* Se tastează run(E), unde E este o expresie formatată după regulile profei:
   <expr> -> <număr> | <var> |
             <var> -> <expr> |
             <expr> $ <expr> |
             <expr> + <expr>
   ...
   ?- run(x -> (y -> (x -> x + z) $ ((x -> x $ x) $ (x + y) + x)) $ (z -> x $ y $ z)).
*/
...
/* legate_de((X, I), L1, L2)
   Descriere: L2 este L1 in care elementele ((X, J), libera) sunt inlocuite cu
   ((X, J), legata(X, I)).
*/
legate_de((_, _), [], []).
